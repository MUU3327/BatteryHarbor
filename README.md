...
> 当前版本为 `0.2` 社区技术预览版。底层充电控制会写入 AppleSMC，仅建议能够理解并完成安全自检的用户在测试设备上使用。不同 Mac 型号与 macOS 版本的兼容性仍需继续验证。
...
| 充电控制策略 | 已完成 | 上限、暂停、临时充满和 3% 回差；常规上限策略不会强制放电 |
...
| 自动化测试 | 39 个单元测试已通过 | CI 只执行普通构建与单元测试，不执行 SMC 写入 |
| 公开发行 | `0.2` 社区技术预览版 | 已采用 MIT；自签名 arm64 DMG 通过静态验签，无 Apple ID/无维护者证书的干净账号验证已通过；Developer ID 公证版暂不提供 |

## 主要功能

- 可拖动的充电上限，达到上限后自动暂停充电
- 暂停/恢复充电与临时充至 100%；常规充电上限不会强制放电
